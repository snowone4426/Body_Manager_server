...
public class AccountServiceImpl implements AccountService {
    ...
    public String orderList(String page, String limit) {
        ...
        List<Tuple> count = jpaQueryFactory.select(dateFormat, orderInfo.order_id).from(orderInfo).where(orderInfo.member.member_id.eq(4l)).offset(Long.parseLong(page)).limit(Long.parseLong(limit)).orderBy(dateFormat.desc()).fetch();
